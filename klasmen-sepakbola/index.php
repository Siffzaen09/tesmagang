...
if (isset($_SESSION['id'])) {
	$menu_input_barang = '
	<li class="nav-item">
		<a class="nav-link" href="klasemen.php">Kelola Klasemen</a>
	</li>
	<li class="nav-item">
...
<body>
	<header>
      <nav class="navbar navbar-expand-lg navbar-dark bg-success box-shadow">
		    		...
		    		<td><?php echo $no++ ?></td>
			      	...
			      	<td>
			      		<?php foreach ($key['performa'] as $key2 => $val) {
			      		    if ($val == 'M') {
			      		    	echo '<button class="btn btn-score btn-small btn-secondary" data-bs-toggle="modal" data-bs-target="#scoreModal" data-score-team="'.$key['score_team'][$key2].'" data-score-lawan="'.$key['score_lawan'][$key2].'">Score</button> <button style="width: 40px; height: 40px" class="mb-1 btn btn-small btn-success">M</button> ';
			      		    } else if ($val == 'I') {
			      		    	echo '<button class="btn btn-score btn-small btn-secondary" data-bs-toggle="modal" data-bs-target="#scoreModal" data-score-team="'.$key['score_team'][$key2].'" data-score-lawan="'.$key['score_lawan'][$key2].'">Score</button> <button style="width: 40px; height: 40px" class="mb-1 btn btn-small btn-secondary">I</button> ';
			      		    } else {
			      		    	echo '<button class="btn btn-score btn-small btn-secondary" data-bs-toggle="modal" data-bs-target="#scoreModal" data-score-team="'.$key['score_team'][$key2].'" data-score-lawan="'.$key['score_lawan'][$key2].'">Score</button> <button style="width: 40px; height: 40px" class="mb-1 btn btn-small btn-danger">K</button> ';
			      		    }
			      		} ?>
			      	</td>
		    	</tr>
